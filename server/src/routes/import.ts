...
import { notAlreadyImporting, validating } from '../utils/middleware'
...
importRouter.post('/upload', upload.array('imports', 50), notAlreadyImporting, async (req, res) => {
...
importRouter.post('/retry', validating(retrySchema), notAlreadyImporting, async (req, res) => {
...
importRouter.delete('/clean/:id', validating(cleanupImportSchema, 'params'), async (req, res) => {
    const { user } = req as LoggedRequest
...
importRouter.get('/state', async (req, res) => {
    const { user } = req as LoggedRequest
